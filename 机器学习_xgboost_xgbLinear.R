...
model<- train(x=data,y=as.factor(group),  method = "xgbLinear", trControl = TrainControl,verbose = FALSE)
...
write.csv(importances,"机器学习_XGBoost_xgbLinear.csv")
...
p1=p+ geom_hline(yintercept = 0, linetype = 2, color = "lightgray")+
  ...
  grids()   
#保存图片
pdf(file="importance_xgboost_xgbLinear.pdf", width=3.5, height=4.5)
print(p1)
dev.off()
save(af,file = "机器学习_xgboost_xgbLinear.Rdata")
